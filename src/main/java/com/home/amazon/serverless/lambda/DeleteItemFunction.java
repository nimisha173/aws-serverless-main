...
public class DeleteItemFunction implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    private final TableSchema<Employee> empTableSchema;

    public DeleteItemFunction() {

        dbClient = DependencyFactory.dynamoDbEnhancedClient();
        tableName = DependencyFactory.tableName();
        empTableSchema = TableSchema.fromBean(Employee.class);
    }

    @Override
    public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent request, Context context) {
        String responseBody = "";
        DynamoDbTable<Employee> empsTable = dbClient.table(tableName,empTableSchema);
        Map<String, String> pathParameters = request.getPathParameters();
        if (pathParameters != null) {
            final String psno = pathParameters.get(Employee.PARTITION_KEY);
            if (psno != null && !psno.isEmpty()) {
                Employee deletedemp = empsTable.deleteItem(Key.builder().partitionValue(psno).build());
                try {
                    responseBody = new ObjectMapper().writeValueAsString(deletedemp);
